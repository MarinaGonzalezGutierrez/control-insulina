...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $fechaHipo = $_POST["fechaHipo"] ?? null;
    $tipoComidaHipo = $_POST["tipoComidaHipo"] ?? null;
    $horaHipo = $_POST["horaHipo"] ?? null;
    $glucosaHipo = $_POST["glucosaHipo"] ?? null;

    if (!isset($fechaHipo) || !isset($tipoComidaHipo) || !isset($horaHipo) || !isset($glucosaHipo)) {
        ...
        $stmtCheckComidas->execute([$idUsuario, $fechaHipo, $tipoComidaHipo]);
        ...
        if ($countComidas == 0) {
            ...
            $stmtInsertComidas->execute([$idUsuario, $fechaHipo, $tipoComidaHipo]);
        }

        // Insertar el nuevo registro en hipo
        $sql = "INSERT INTO hipo (idUsuario, fecha, tipoComida, hora, glucosa) VALUES (?, ?, ?, ?, ?)";
        $stmt = $pdo->prepare($sql);

        if ($stmt->execute([$idUsuario, $fechaHipo, $tipoComidaHipo, $horaHipo, $glucosaHipo])) {
...
<body>
    <div class="container-fluid d-flex justify-content-center align-items-center bg-degradado-custom" style="min-height: 100vh;">
        ...
        <div class="col-12 col-sm-8 col-md-6 col-lg-4 bg-white p-4 rounded shadow-lg">
            <!-- AÑADIR COMIDA -->
            <form method="POST">
                <h3 class="text-center mb-3">Hipoglucemia</h3>

                <div class="row mb-3">
                    <label for="fechaHipo" class="col-form-label">Fecha</label>
                    <input type="date" class="form-control" id="fechaHipo" name="fechaHipo" required>
                </div>

                <div class="row mb-3">
                    <label for="tipoComidaHipo" class="col-form-label">Tipo de comida</label>
                    <select class="form-select" id="tipoComidaHipo" name="tipoComidaHipo">
                ...
                <div class="row mb-3">
                    <label for="horaHipo" class="col-form-label">Hora</label>
                    <input type="time" class="form-control" id="horaHipo" name="horaHipo" required>
                </div>
